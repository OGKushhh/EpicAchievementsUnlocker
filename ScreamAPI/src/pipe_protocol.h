...
struct PktHeader {
    uint32_t magic;        // always ABDO_MAGIC — detects packing/endian mismatches
    PktType  type;
...
// Payload: AchListHeader followed by `count` AchEntry records,
//          then the string blob (all strings null-terminated, packed together).
//
// Each AchEntry has byte offsets into the blob for id/name/desc.

struct AchListHeader {
    uint32_t count;
    uint32_t blobSize;   // total bytes in string blob
};

struct AchEntry {
    uint32_t idOff;      // offset into string blob
    uint32_t nameOff;
    uint32_t descOff;
    uint8_t  isHidden;
